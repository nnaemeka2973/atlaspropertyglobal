(() => {
  ...
  const photos = Array.isArray(property.photos)
    ? [
        ...new Set(
          property.photos
            .filter(Boolean)
            .map(src => String(src).replace(/^http:\/\//i, "https://").replace(/s\.jpg$/i, "o.jpg"))
        )
      ]
    : [];
  ...
  const propertyLabel =
    property.title || property.address || "this property";
  ...
  const unavailable = title => section(title, [
  ...
  const findAdvertiser = value => {
    ...
    if (Array.isArray(value.advertisers) && value.advertisers.length) {
      const preferred =
        value.advertisers.find(a => a?.type === "seller") ||
        value.advertisers[0];
  ...
  const seed = String(property.id || property.address || "atlas")
    .split("")
    .reduce(
  ...
  const fallbackAgentName = agentPool[seed % agentPool.length];

  const agentName =
    suppliedAdvertiser?.name || fallbackAgentName;
  ...
  if (property.listingDate) {
    history.push(["Listing date", property.listingDate]);
  }

  if (property.lastSoldDate) {
    history.push(["Last sold date", property.lastSoldDate]);
  }

  if (property.lastSoldPrice != null) {
    history.push([
      "Last sold price",
      formatPropertyPrice(property.lastSoldPrice)
    ]);
  }
  ...
  if (property.priceReducedAmount != null) {
    history.push([
      "Price reduction",
      formatPropertyPrice(property.priceReducedAmount)
    ]);
  }
  ...
  const emailTarget = agentEmail || "";
   ...
   const whatsappNumber = "13346971225"; // +1 (334) 697-1225
...
Thank you.`
  ...
  const insuranceMonthly =
    Math.round((listingPrice * 0.004) / 12);
  ...
  const aiAnswer = question => {
    ...
    if (/size|square|sqft|feet/.test(q)) {
    ...
    if (/mortgage|payment|monthly/.test(q)) {
    ...
    if (/location|where|address/.test(q)) {
      return addressText
    ...
    if (
      /feature|amenit|inside|interior|outside|exterior/.test(
        q
      )
    ) {
      const items = [
  ...
  root.innerHTML = `
    <section class="container details-top">
      ...
      <a class="back-link" href="properties.html">
        ← Back to properties
      </a>

      <div class="hero-gallery">

        <div class="main-photo" data-photo-index="${initialIndex}">
    ...
    <section class="container detail-layout">
      ...
      <div class="content-column">
        ...
        <div class="main-column">
          ...
          ${section("Property Highlights", d.propertyHighlights)}

          ${d.propertyFeatures.length ? section("Property Features", d.propertyFeatures) : unavailable("Property Features")}

          ${section("Location Information", d.location)}

          ${section("Financial Information", d.financialInformation)}

          ${d.nearbyPlaces.length ? section("Nearby Places", d.nearbyPlaces) : unavailable("Nearby Places")}

          ${d.commuteInformation.length ? section("Commute Information", d.commuteInformation) : unavailable("Commute Information")}

          ${d.floorPlans.length ? section("Floor Plans", d.floorPlans) : unavailable("Floor Plans")}

          ${d.documents.length ? section("Documents", d.documents) : unavailable("Documents")}

          ${d.sustainability.length ? section("Sustainability", d.sustainability) : unavailable("Sustainability")}
    ...
    >
      <div class="mobile-contact-left">
        <span class="mobile-contact-avatar" aria-hidden="true">
        ...
        <span class="mobile-contact-copy">
          ...
          <span class="mobile-contact-role">
            ${isFormulatedAgent ? "Atlas Property Group listing specialist" : "Listing agent"}
          </span>
        </span>
      </div>
    ...
    ?.addEventListener(
      ...
      async event => {
        ...
        try {
           ...
           * sent to the backend instead of
           * the frontend server.
           */
